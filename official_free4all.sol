...
contract FREE4ALL is Context, IERC20, Ownable {
    ...
    string private _name = "FREE4ALL";
    string private _symbol = "F4X";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 5;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 10; //(3% liquidityAddition + 2% rewardsDistribution + 5% taxfee)
    ...
    function _transfer(
        ...
        if(limit ==  true && from != owner() && to != owner()){
            if(to != pancakePair){
                require(((balanceOf(to).add(amount)) <= 100 ether));
            }
  ...
  //TIMER
    ...
    function getTimeLeft() public view returns(uint256){
     ...
     function Time() public returns(uint256) {
            ...
            if (now <= createTime + 10){
            return createTime;
            
             }
            else if (now > createTime + 20)  {
            return createTime += 20;
    ...
    function isOpen()private returns(bool){
        Time();
        return (now >= createTime + 10 && now <= createTime + 20);
        
    }

    //locked always except for 2mins(120seconds) between 6 hours
    modifier onlyWhileOpen(){ 
        Time();
        require(now >= createTime + 10);
        require(now <= createTime + 20);
        _;
    ...
    function swapTokensForEth(uint256 tokenAmount) private onlyWhileOpen {
